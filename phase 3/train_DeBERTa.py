"""
DM2025 Lab 2 - Phase 3: DeBERTa-v3 Model Training
專門訓練 DeBERTa 模型用於 ensemble
"""
...
from transformers import (
    DebertaV2Tokenizer,  # 新增：強制使用慢速 tokenizer
...
def train_with_cross_validation(train_df, config):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_df, train_df['label'])):
        ...
        training_args = TrainingArguments(
            output_dir=f"./results_deberta/fold_{fold}",
            ...
            logging_dir=f'./logs_deberta/fold_{fold}',
            logging_steps=100,
            eval_strategy="epoch",
            save_strategy="epoch",
        ...
        os.makedirs('./deberta_models', exist_ok=True)
        model.save_pretrained(f'./deberta_models/fold_{fold}')
        tokenizer.save_pretrained(f'./deberta_models/fold_{fold}')
...
def main():
    print("="*70)
    print("DM2025 Lab 2 - DeBERTa-v3-base Training")
    print("="*70)
    
    print(f"\nModel: {Config.MODEL_NAME}")
    print(f"Purpose: Ensemble with Twitter-RoBERTa")
    ...
    print(f"{'='*70}")
    print(f"\nResults:")
    print(f"  DeBERTa OOF F1: {oof_f1:.4f}")
    print(f"\nOutput:")
    print(f"  Models saved in: ./deberta_models/fold_0-4/")
    print(f"\nNext Step:")
    print(f"  Run ensemble_predict.py to combine with Twitter-RoBERTa")
    print(f"{'='*70}")
